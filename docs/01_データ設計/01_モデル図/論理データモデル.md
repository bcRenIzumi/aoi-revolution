...
| ---------- | ---------- | --------- | :---: | :---: | :------: | -------------------------- |
| ID         | id         | nvarchar  |   ○   |   -   |    ×     | 企業を一意に識別する業務ID |
| 企業名     | name       | nvarchar  |   -   |   -   |    ×     | 企業の正式名称             |
...
| ------------------ | ---------------- | --------- | :---: | :---: | :------: | ------------------------------------- |
| 申請ID             | app_id           | nvarchar  |   ○   |   -   |    ×     | 申請ID (T_APP_INFO.id)                |
| 管理会社ID         | company_id       | nvarchar  |   -   |   ○   |    ×     | 管理会社ID (M_COMPANY.id)             |
| 希望納品日ID       | delivery_date_id | nvarchar  |   -   |   ○   |    ×     | 希望納品日ID (M_DELIVERY_DATE.id)     |
| 管理者部門名       | manager_dept     | nvarchar  |   -   |   -   |    ×     | 管理者の部門名                        |
| 管理者ID           | manager_id       | nvarchar  |   -   |   ○   |    ×     | 管理者ID (M_EMPLOYEE.id)              |
| 管理者氏名         | manager_name     | nvarchar  |   -   |   -   |    ×     | 管理者の氏名                          |
| 費用負担先コード   | cost_code        | nvarchar  |   -   |   -   |    ○     | 指定費用負担先コード                  |
| 使用者部門名       | user_dept        | nvarchar  |   -   |   -   |    ○     | 使用者の部門名                        |
| 使用者ID           | user_id          | nvarchar  |   -   |   ○   |    ○     | 使用者ID (M_EMPLOYEE.id)              |
| 使用者氏名         | user_name        | nvarchar  |   -   |   -   |    ○     | 使用者の氏名                          |
| 設置拠点ID         | location_id      | nvarchar  |   -   |   ○   |    ×     | 設置拠点ID (M_LOCATION.id)            |
| 設置フロア         | floor            | nvarchar  |   -   |   -   |    ○     | 設置フロア                            |
| 設置拠点その他     | other_location   | nvarchar  |   -   |   -   |    ○     | その他の設置拠点詳細                  |
| 設置場所詳細       | location_detail  | nvarchar  |   -   |   -   |    ○     | 設置場所の詳細情報                    |
| ネットワーク環境ID | network_env_id   | nvarchar  |   -   |   ○   |    ×     | ネットワーク環境ID (M_NETWORK_ENV.id) |
| 指定IPアドレス     | ip_address       | nvarchar  |   -   |   -   |    ○     | 指定IPアドレス                        |
| 納品先社名ID       | dlv_company_id   | nvarchar  |   -   |   ○   |    ×     | 納品先の社名ID (M_COMPANY.id)         |
| 納品先拠点ID       | dlv_location_id  | nvarchar  |   -   |   ○   |    ×     | 納品先の拠点ID (M_LOCATION.id)        |
| 納品先郵便番号     | dlv_postal       | nvarchar  |   -   |   -   |    ○     | 納品先の郵便番号                      |
| 納品先住所         | dlv_address      | nvarchar  |   -   |   -   |    ○     | 納品先の住所                          |
| 納品先フロア       | dlv_floor        | nvarchar  |   -   |   -   |    ○     | 納品先のフロア                        |
| 納品先部門名       | dlv_dept         | nvarchar  |   -   |   -   |    ○     | 納品先の部門名                        |
| 受取担当者名       | receiver         | nvarchar  |   -   |   -   |    ×     | 受取担当者の氏名                      |
| 受取担当者電話番号 | receiver_phone   | nvarchar  |   -   |   -   |    ×     | 受取担当者の電話番号                  |
| 導入区分ID         | install_type_id  | nvarchar  |   -   |   ○   |    ○     | 導入区分ID (M_INSTALLATION_TYPE.id)   |
| PCモデルID         | model_id         | nvarchar  |   -   |   ○   |    ×     | PCモデルID (M_PC_MODEL.id)            |
| 申請台数           | quantity         | int       |   -   |   -   |    ×     | 申請PC台数                            |
| 申請理由           | reason           | nvarchar  |   -   |   -   |    ×     | 申請理由                              |
| 現行PC管理番号     | current_pc_no    | nvarchar  |   -   |   -   |    ○     | 現行PCの管理番号                      |
| 現行PC契約終了日   | current_pc_end   | date      |   -   |   -   |    ○     | 現行PCの契約終了日                    |
| 申請状態           | status           | nvarchar  |   -   |   -   |    ×     | 申請の現在の状態                      |
| 申請日             | request_date     | date      |   -   |   -   |    ×     | 申請が行われた日付                    |
| 承認日             | approval_date    | date      |   -   |   -   |    ○     | 申請が承認された日付                  |
| 承認者ID           | approver_id      | nvarchar  |   -   |   ○   |    ○     | 承認者ID (M_EMPLOYEE.id)              |
| 有効フラグ         | is_active        | bit       |   -   |   -   |    ×     | true: 有効、false: 無効               |
| 作成日時           | created_at       | datetime2 |   -   |   -   |    ×     | 作成日時                              |
| 更新日時           | updated_at       | datetime2 |   -   |   -   |    ×     | 更新日時                              |
